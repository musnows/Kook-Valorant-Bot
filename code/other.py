...
async def history(msg: Message):
    ...
    c1 = Card(Module.Header(f'历史上的{now_date}'),
              Module.Context('以古为镜,可以知兴替...'))
    c1.append(Module.Divider())
    for its in json_dict['data']:
        c1.append(
            Module.Section(
                f"{its['year']}年，{its['title']}",
                Element.Button('查看', f"{its['link']}", Types.Click.LINK)))
        c1.append(Module.Divider())
...
async def weather(msg: Message, city: str):
    ...
    c1 = Card(
        Module.Header(
            f"已为您查询 {json_dict['city']} 的天气，更新于{json_dict['update_time']}"),
    ...
    for its in json_dict['weather']:
        if i > 0:
            c1.append(
                Module.Section(
                    f"{its['date']}  天气: {its['weather']}  温度: {its['temp']}  风向: {its['wind']}",
                ))
            c1.append(Module.Divider())
            i -= 1
        else:
