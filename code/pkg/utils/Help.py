...
def help_main(start_time: str):
    ...
    text += "公告：因为拳头公司对API添加了人机验证和IP访问频次防火墙，导致API接口已不可用。阿狸已停止所有需要登录的功能，仅提供原有娱乐功能以供使用。\n"
...
def help_develop():
    ...
    text += f"「/log」显示当前机器人加入的服务器以及用户数量\n"
    text += f"「/mem」显示当前机器人进程的内存/cpu占用\n"
