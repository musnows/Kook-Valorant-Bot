...
| /shop-img  | 登录，直接返回并跳转商店的图片，无缓存  | 正常   |
| /shop  | 登录，以`json`格式返回商店图片url或者riot原始响应  | 正常   |
| /login | 登录接口，需和`/shop`接口配合使用 | 正常   |
| /tfa  | 邮箱验证接口，需和`/shop`接口配合使用 | 正常   |
...
您需要先请求 `/shop` 接口，在用户获取到验证码后，再请求本接口；若在10min内没有收到 `/tfa` 接口请求，后台会以**邮箱验证超时**关闭该账户的会话（对应报错为 `2fa wait overtime`）。
