...
* cpu：至少1核
* 硬盘：120mb
* 内存：400mb（不在内存中缓存图片）/ 900mb（内存缓存图片）
...
保证你的 `Python` 版本高于 `3.10`，执行下面的安装库命令
...
更新Python版本的方法可以参考本仓库wiki的 [01-How_to_update_Python](https://github.com/musnows/Valorant-Kook-Bot/wiki/01-How_to_update_Python)
...
<details>
<summary>floxay/python-riot-auth</summary>

> 注意：新版本已不再依赖于此项目，可忽略如下内容！
...
> 示例配置文件参考 [config.exp.json](../code/config/config.exp.json)

总结一下，本仓库需要手动配置的文件有下面两个，其中`color_emoji.json`在仓库中有示例文件

| 文件路径                    | 功能                                                         |
| --------------------------- | ------------------------------------------------------------ |
| [code/config/config.json](../code/config/config.exp.json)     | 存放本机器人的基本配置：[KOOK开发者页面](https://developer.kaiheila.cn/doc/intro) |
| [code/config/color_emoji.json](https://github.com/Valorant-Shop-CN/Kook-Valorant-Bot/blob/develop/code/config/color_emoji.json) | 存放自动给用户上角色功能相关的emoji以及 `msg_id/guild_id`       |
...
<font color=orange>提醒</font>：若您的可用内存不足1Gb，请将`config.json`中的`["cache"]["shop_img"]["memory"]`字段设置为false，以禁用商店画图模块的内存缓存
...
不同log文件存放内容参考 [file-tree](./file-tree.md) 中的注释
...
当你想让机器人下岗的时候，可以使用下面这个linux命令

```
kill -9 进程号
```

或者在kook中使用**机器人命令**（墙裂推荐）
